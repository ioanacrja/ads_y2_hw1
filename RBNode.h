...
struct RBTree {
	 ...
	 */
	int bh() {
    if (isNil(root)) return 0; // If tree is empty, black-height is 0
	...
	int maxBlackKeyHelper(RBNode* x) {
    if (isNil(x)) return -1000;
    int maxLeft = maxBlackKeyHelper(x->left);
    int maxRight = maxBlackKeyHelper(x->right);
    int currentMax = (x->col == RBNode::BLACK) ? x->key : -1000;
    return max(currentMax, max(maxLeft, maxRight));
}

int maxBlackKey() {
    if (isNil(root)) return -1000; // If tree is empty
    return maxBlackKeyHelper(root);
}

	/* return the maximum key of red nodes.
	 * If the tree has no red nodes, return -1000 */
	int maxRedKeyHelper(RBNode* x) {
    if (isNil(x)) return -1000;
    int maxLeft = maxRedKeyHelper(x->left);
    int maxRight = maxRedKeyHelper(x->right);
    int currentMax = (x->col == RBNode::RED) ? x->key : -1000;
    return max(currentMax, max(maxLeft, maxRight));
}

int maxRedKey() {
    if (isNil(root)) return -1000; // If tree is empty or no red nodes
    return maxRedKeyHelper(root);
}
};
